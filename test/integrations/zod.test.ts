...
describe('Zod → JSON Schema Converter', () => {
  ...
  it('converts string with constraints', () => {
    const [, schema] = converter.convert(z.string().min(3).max(100).email() as any, { strategy: 'input' })
    expect(schema.type).toBe('string')
  ...
  it('converts number with constraints', () => {
    const [, schema] = converter.convert(z.number().min(0).max(100) as any, { strategy: 'input' })
  ...
  it('converts object with required/optional fields', () => {
    const [, schema] = converter.convert(z.object({ name: z.string(), age: z.number().optional() }) as any, {
      strategy: 'input',
    })
  ...
  it('converts array with min/max', () => {
    const [, schema] = converter.convert(z.array(z.number()).min(1).max(10) as any, { strategy: 'input' })
    expect(schema.minItems).toBe(1)
    expect(schema.maxItems).toBe(10)
  })

  it('converts enum', () => {
    const [, schema] = converter.convert(z.enum(['active', 'inactive', 'banned']) as any, { strategy: 'input' })
    expect(schema.enum).toEqual(['active', 'inactive', 'banned'])
  })

  it('converts union (anyOf)', () => {
    const [, schema] = converter.convert(z.union([z.string(), z.number()]) as any, { strategy: 'input' })
  ...
  it('converts default (required=false, has default)', () => {
    const [req, schema] = converter.convert(z.string().default('hello') as any, { strategy: 'input' })
    expect(req).toBe(false)
  ...
  it('converts tuple', () => {
    const [, schema] = converter.convert(z.tuple([z.string(), z.number()]) as any, { strategy: 'input' })
    expect(schema.type).toBe('array')
    expect(schema.prefixItems).toHaveLength(2)
  })

  it('converts record', () => {
    const [, schema] = converter.convert(z.record(z.string(), z.number()) as any, { strategy: 'input' })
    expect(schema.type).toBe('object')
