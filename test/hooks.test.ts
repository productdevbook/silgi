...
function createApp(hooks?: Parameters<typeof katman>[0]["hooks"]) {
  ...
  const router = k.router({
    ...
    fail: k.query(() => { throw new Error("boom"); }),
  });
...
describe("lifecycle hooks", () => {
  it("calls request hook before processing", async () => {
    ...
    const { handle } = createApp({
      request: ({ path }) => { events.push(`req:${path}`); },
    });
  ...
  it("calls response hook after success", async () => {
    const events: any[] = [];
    const { handle } = createApp({
      response: (e) => { events.push({ path: e.path, output: e.output }); },
    });
  ...
  it("response hook includes durationMs", async () => {
    let duration = -1;
    const { handle } = createApp({
      response: (e) => { duration = e.durationMs; },
    });
  ...
  it("k.removeHook() removes a specific hook", async () => {
    ...
    const fn = ({ path }: { path: string }) => { events.push(path); };
    k.hook("request", fn);
