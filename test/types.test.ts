  ...
  it("fail with data schema requires typed second arg", () => {
    k.query({
      ...
      resolve: ({ fail }) => {
        ...
        expectTypeOf(fail<"VALIDATION">).parameters.toEqualTypeOf<
          ["VALIDATION", { field: string; message: string }]
        >();
        // fail("NOT_FOUND") should NOT require data (number shorthand = no data schema)
        expectTypeOf(fail<"NOT_FOUND">).parameters.toEqualTypeOf<
          ["NOT_FOUND", ...([data?: unknown] | [])]
        >();
        return true;
      },
    });
