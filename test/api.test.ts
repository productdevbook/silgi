...
describe("wrap()", () => {
  ...
  it("guards run before wraps", async () => {
    const order: string[] = [];
    const g = k.guard(() => { order.push("guard"); return { x: 1 }; });
    ...
    const proc = k.query({
      use: [g, w],
      resolve: async () => { order.push("handler"); return "ok"; },
    });
...
describe("query() / mutation() / subscription()", () => {
  ...
  it("short form: query(input, resolve)", () => {
    const proc = k.query(
      ...
      async ({ input }: any) => `hello ${input.name}`,
    );
    expect(proc.type).toBe("query");
...
describe("handler()", () => {
  const router = k.router({
    ...
    greet: k.query(
      z.object({ name: z.string() }) as any,
      async ({ input }: any) => ({ message: `Hello, ${input.name}!` }),
    ),
    fail: k.mutation({
      errors: { FORBIDDEN: 403 },
      resolve: async ({ fail }: any) => { fail("FORBIDDEN"); },
    }),
  });
...
describe("E2E: Full CRUD", () => {
  ...
  const router = k2.router({
    users: {
      list: k2.query(
        z.object({ limit: z.number().optional() }) as any,
        async ({ input }: any) => db.slice(0, input?.limit ?? 10),
      ),
      create: k2.mutation({
        use: [auth],
  ...
  it("creates user with auth", async () => {
    const res = await handle(new Request("http://localhost/users/create", {
      method: "POST",
