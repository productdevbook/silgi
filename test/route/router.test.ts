 ...
 * `undefined`).
 */
function assertLookups(router: ReturnType<typeof createRouter<{ path: string }>>, tests: TestRoutes) {
...
function testRouter(paths: string[], tests?: TestRoutes) {
  ...
  const resolvedTests: TestRoutes = tests ?? Object.fromEntries(paths.map((p) => [p, { data: { path: p } }]))
...
describe('empty segments', () => {
  it('double slash segment', () => {
    testRouter(['/test//route', '/test/:param/route'], {
      '/test//route': {
...
describe('parametric routes', () => {
  ...
  it('params between static segments', () => {
    testRouter(['/this/:route/has/:cool/stuff'], {
  ...
  it('mixed param routes together', () => {
    testRouter(['/blog/*', '/carbon/:element', '/carbon/:element/test/:testing', '/this/:route/has/:cool/stuff'], {
      '/carbon/test1': {
        data: { path: '/carbon/:element' },
        params: { element: 'test1' },
      },
  ...
  it('hyphenated param names', () => {
    testRouter(['/users/:user-id', '/users/:user-id/posts/:post-id', '/items/:item-name/details'], {
      '/users/123': {
...
describe('wildcard routes', () => {
  ...
  it('fallback to wildcard from static', () => {
    testRouter(['/wildcard/**', '/test/**', '/test', '/dynamic/*'], {
      '/wildcard': {
        data: { path: '/wildcard/**' },
      },
      '/wildcard/': {
...
describe('mixed params in same segment', () => {
  ...
  it('prefix @:param coexists with :param', () => {
    testRouter(['/npm/:param1/:param2', '/npm/@:param1/:param2'], {
      '/npm/@test/123': {
...
describe('regex constraints (named)', () => {
  ...
  it('enum param :ext(png|jpg|gif)', () => {
    testRouter(['/files/:ext(png|jpg|gif)'], {
      '/files/png': {
  ...
  it('versioned API :version(v\\d+)/:resource', () => {
    testRouter(['/api/:version(v\\d+)/:resource'], {
      '/api/v2/users': {
  ...
  it('regex + unconstrained coexist', () => {
    testRouter(['/users/:id(\\d+)', '/users/:slug'], {
      '/users/123': {
        data: { path: '/users/:id(\\d+)' },
        params: { id: '123' },
      },
