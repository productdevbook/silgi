 ...
 */
export function procedureToTool(
  name: string,
  ...
  const description = options?.description
    ?? (procedure.route as any)?.description
    ?? (procedure.route as any)?.summary
    ?? `Call ${name}`;
...
function collectProcedures(
  ...
  if (isProcedureDef(node)) { cb(path, node); return; }
...
function convertDef(def: any): Record<string, unknown> {
  ...
  switch (type) {
    ...
    case "array": return { type: "array", ...(def.element ? { items: zodToJsonSchemaSimple(def.element) } : {}) };
    case "optional": return zodToJsonSchemaSimple(def.innerType ?? def.inner);
    case "enum": return { type: "string", enum: def.values ?? def.entries };
    default: return {};
  }
}
