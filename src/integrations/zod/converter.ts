...
export class ZodSchemaConverter implements SchemaConverter {
  ...
  #convert(schema: any, options: ConvertOptions, depth: number): [boolean, JSONSchema] {
    ...
    switch (typeName) {
      ...
      case "tuple":
      case "ZodTuple": {
        const items = (def.items ?? def.types ?? []).map(
          (t: any) => this.#convert(t, options, depth + 1)[1],
        );
      ...
      case "record":
      case "ZodRecord": {
        const [, valSchema] = this.#convert(
          def.valueType ?? def.element, options, depth + 1,
        );
  ...
  #convertString(def: any, bag?: any): JSONSchema {
    ...
    for (const check of checks) {
      switch (check.kind ?? check.type) {
        ...
        case "regex": result.pattern = check.regex?.source ?? check.value; break;
        case "datetime": result.format = "date-time"; break;
        case "date": result.format = "date"; break;
        case "time": result.format = "time"; break;
        case "ip": result.format = check.version === "v6" ? "ipv6" : "ipv4"; break;
      }
    }
  ...
  #hasIntCheck(def: any): boolean {
    ...
    return checks.some((c: any) =>
      (c.kind ?? c.type ?? c._zod?.def?.check) === "int" ||
      c.isInt === true ||
      c._zod?.def?.format === "safeint",
    );
  }
  ...
  #convertNumber(def: any, bag?: any): JSONSchema {
    ...
    for (const check of checks) {
      switch (check.kind ?? check.type) {
        ...
        case "finite": break; // JSON numbers are always finite
        case "nonnegative": result.minimum = 0; break;
        case "nonpositive": result.maximum = 0; break;
        case "positive": result.exclusiveMinimum = 0; break;
        case "negative": result.exclusiveMaximum = 0; break;
      }
    }
