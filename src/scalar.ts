      ...
      if (method === "get") {
        ...
        operation.requestBody = {
          ...
          content: {
            ...
            "application/x-msgpack": { schema: { type: "string", format: "binary", description: "MessagePack encoded" } },
          },
        };
      }
    }
    ...
    if (proc.output) {
      ...
      (operation.responses as any)[String(successStatus)] = {
        ...
        content: {
          ...
          "application/x-devalue+json": { schema: { type: "string", description: "devalue-encoded (Date, Map, Set, BigInt)" } },
        },
      };
    } else {
...
function applyStringChecks(schema: JSONSchema, checks?: any[]): JSONSchema {
  ...
  for (const c of checks) {
    ...
    if (k === "length") { schema.minLength = c.value; schema.maxLength = c.value; }
