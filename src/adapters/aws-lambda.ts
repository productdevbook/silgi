  ...
  return async (event: LambdaEvent): Promise<LambdaResponse> => {
    ...
    try {
      ...
      if (event.body) {
        ...
        try { input = JSON.parse(event.queryStringParameters.data); } catch { /* ignore */ }
      }
      ...
      if (error instanceof ValidationError) {
        return {
          statusCode: 400,
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ code: "BAD_REQUEST", status: 400, message: error.message, data: { issues: error.issues } }),
        };
      }
