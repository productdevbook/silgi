...
async function benchSilgi(): Promise<Result> {
  ...
  const url = `http://127.0.0.1:${server.port}/users/list`
  await verify(url)
...
async function benchElysia(): Promise<Result> {
  const app = new Elysia().post('/users/list', ({ body }) => {
    const limit = (body as any)?.limit ?? 10
    return { users: makeUsers(limit) }
  }).listen(0)
  const url = `http://127.0.0.1:${app.server!.port}/users/list`
  await verify(url)
  const result = await measure(url, REQUESTS)
  app.stop()
  return result
...
console.log(`\nHTTP Benchmark (Bun ${Bun.version}) — ${REQUESTS} requests × ${ROUNDS} rounds (${WARMUP} warmup each)\n`)
