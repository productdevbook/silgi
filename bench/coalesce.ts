...
async function startServer(port: number, mode: 'plain' | 'cached' | 'full'): Promise<Server> {
  ...
  return new Promise((r) => {
    const s = createServer(async (req, res) => {
      ...
      try {
        const body = coalescer ? await coalescer.execute(cacheKey, execute) : await execute()
...
async function startORPCServer(): Promise<Server> {
  const orpcRouter = {
    list: orpcOs.input(z.object({ limit: z.number() })).handler(async ({ input }) => {
...
async function main() {
  ...
  for (const conc of [10, 50, 100, 200]) {
    ...
    console.log(
      `  Katman (plain):          ${kPlain.rps.toLocaleString().padStart(7)} req/s  (${handlerCallCount.kPlain} handler calls)`,
    )
    console.log(
      `  Katman (cache):          ${kCached.rps.toLocaleString().padStart(7)} req/s  (${handlerCallCount.kCached} handler calls)`,
    )
    console.log(
      `  Katman (cache+coalesce): ${kFull.rps.toLocaleString().padStart(7)} req/s  (${handlerCallCount.kFull} handler calls)`,
    )
    console.log(
      `  oRPC:                    ${oRpc.rps.toLocaleString().padStart(7)} req/s  (${handlerCallCount.orpc} handler calls)`,
    )
    console.log(`  → Katman full ${speedup}x faster than oRPC`)
    console.log(
      `  → Handler calls: Katman ${handlerCallCount.kFull} vs oRPC ${handlerCallCount.orpc} (${(handlerCallCount.orpc / Math.max(1, handlerCallCount.kFull)).toFixed(0)}x less work)\n`,
    )
  }
