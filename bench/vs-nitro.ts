...
async function bench(
  ...
  // Warmup
  for (let i = 0; i < 200; i++) {
    try { await (await fetch(url, opts)).text(); } catch {}
  }
...
async function main() {
  ...
  try {
    ...
    for (const s of scenarios) {
      ...
      if (!kRes.ok) { console.error(`Katman ${s.name} failed: ${kRes.status} ${await kRes.text()}`); }
      if (!nRes.ok) { console.error(`Nitro ${s.name} failed: ${nRes.status} ${await nRes.text()}`); }
      await kRes.text();
      await nRes.text();
    }
    ...
    for (const r of results) {
      ...
      console.log(`    Nitro  : avg ${fmt(r.nitro.avg)} | p50 ${fmt(r.nitro.p50)} | p99 ${fmt(r.nitro.p99)} | ${r.nitro.rps} req/s`);
    }
