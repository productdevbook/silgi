...
async function startKatman(): Promise<Server> {
  ...
  const router = k.router({
    ...
    guarded: k.mutation({ use: [auth], input: GuardedInput as any, resolve: ({ input, ctx }: any) => ({ name: input.name, by: ctx.userId }) }),
  });
    ...
    const server = createServer(async (req, res) => {
      ...
      try {
        ...
        if (req.method !== "GET" && req.headers["content-length"]) {
          const text: string = await new Promise(r => { let b = ""; req.on("data", (d: Buffer) => b += d); req.on("end", () => r(b)); });
...
async function startORPC(): Promise<Server> {
  const router = {
    ...
    guarded: os.use(async ({ next }) => next({ context: { userId: 1 } })).input(GuardedInput).handler(({ input, context }) => ({ name: input.name, by: (context as any).userId })),
  };
...
async function startH3(): Promise<Server> {
  ...
  app.all("/echo", async (event: any) => { const body = await event.req.json(); return { echo: EchoInput.parse(body).message }; });
  app.all("/guarded", async (event: any) => { const body = await event.req.json(); return { name: GuardedInput.parse(body).name, by: 1 }; });
  ...
  return new Promise(resolve => {
    const server = createServer(async (req, res) => {
      ...
      const request = new Request(url, { method: req.method, headers, body: req.method !== "GET" ? body || undefined : undefined });
...
async function startHono(): Promise<Server> {
  ...
  app.post("/echo", async (c) => { const body = await c.req.json(); return c.json({ echo: EchoInput.parse(body).message }); });
  app.post("/guarded", async (c) => { const body = await c.req.json(); return c.json({ name: GuardedInput.parse(body).name, by: 1 }); });
...
async function bench(url: string, body: string | null, n: number) {
  ...
  for (let i = 0; i < n; i++) { const s = performance.now(); await (await fetch(url, opts)).text(); times.push(performance.now() - s); }
  const total = performance.now() - t0;
  times.sort((a, b) => a - b);
  return { avg: times.reduce((a, b) => a + b) / n, rps: Math.round((n / total) * 1000), p50: times[Math.floor(n * 0.5)]!, p99: times[Math.floor(n * 0.99)]! };
}

function fmt(ms: number) { return ms < 1 ? `${(ms * 1000).toFixed(0)}µs` : `${ms.toFixed(2)}ms`; }
...
async function main() {
  ...
  console.log(`Full HTTP Benchmark — ${ITERATIONS} sequential requests | Node ${process.version} | ${new Date().toISOString().split("T")[0]}\n`);
  ...
  for (const r of results) {
    console.log(`  ${r.name}:`);
    console.log(`    Katman: avg ${fmt(r.katman.avg)} | p50 ${fmt(r.katman.p50)} | p99 ${fmt(r.katman.p99)} | ${r.katman.rps} req/s`);
    console.log(`    oRPC:   avg ${fmt(r.orpc.avg)} | p50 ${fmt(r.orpc.p50)} | p99 ${fmt(r.orpc.p99)} | ${r.orpc.rps} req/s`);
    console.log(`    H3 v2:  avg ${fmt(r.h3.avg)} | p50 ${fmt(r.h3.p50)} | p99 ${fmt(r.h3.p99)} | ${r.h3.rps} req/s`);
    console.log(`    Hono:   avg ${fmt(r.hono.avg)} | p50 ${fmt(r.hono.p50)} | p99 ${fmt(r.hono.p99)} | ${r.hono.rps} req/s`);
    console.log(`    → Katman vs oRPC: ${(r.orpc.avg / r.katman.avg).toFixed(1)}x | vs H3: ${(r.h3.avg / r.katman.avg).toFixed(1)}x | vs Hono: ${(r.hono.avg / r.katman.avg).toFixed(1)}x`);
    console.log();
  }

  katmanSrv.close(); orpcSrv.close(); h3Srv.close(); honoSrv.close();
}
