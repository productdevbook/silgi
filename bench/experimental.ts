...
function syncGuard1(ctx: Record<string, unknown>) {
  ctx.a = 1
}
function syncGuard2(ctx: Record<string, unknown>) {
  ctx.b = 2
}
function syncGuard3(ctx: Record<string, unknown>) {
  ctx.c = 3
}
function syncHandler(ctx: any) {
  return { sum: ctx.a + ctx.b + ctx.c }
}
...
function compileSyncPipeline(guards: Array<(ctx: Record<string, unknown>) => void>, handler: Function) {
  ...
  switch (n) {
    case 0:
      return (ctx: any, input: unknown) => h(ctx, input)
    case 1:
      return (ctx: any, input: unknown) => {
        g[0]!(ctx)
        return h(ctx, input)
      }
    case 2:
    ...
    default:
      return (ctx: any, input: unknown) => {
        for (const fn of g) fn(ctx)
        return h(ctx, input)
      }
  }
}

const compiledSync = compileSyncPipeline([syncGuard1, syncGuard2, syncGuard3], syncHandler)
...
const currentWithZod = compileProcedure({
  ...
  output: null,
  errors: null,
  use: [{ kind: 'guard', fn: () => ({ userId: 1 }) } as GuardDef],
