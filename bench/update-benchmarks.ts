...
async function runPipelineBenchmarks(): Promise<PipelineResult[]> {
  ...
  h3App.all('/zod', async (ev: any) => {
    const b = await readBody(ev)
    return InputSchema.parse(b)
  })
  h3App.all('/3mw', async (ev: any) => {
    const b = await readBody(ev)
    return InputSchema.parse(b)
  })
  h3App.all('/5mw', async (ev: any) => {
    const b = await readBody(ev)
    return InputSchema.parse(b)
  })
...
async function runHTTPBenchmarks(): Promise<HTTPResult[]> {
  ...
  h3App.all('/echo', async (ev: any) => {
    const b = await readBody(ev)
    return { echo: EchoInput.parse(b).message }
  })
  h3App.all('/guarded', async (ev: any) => {
    const b = await readBody(ev)
    return { name: GuardedInput.parse(b).name, by: 1 }
  })
...
async function runH2Benchmarks(): Promise<H2Result[]> {
  ...
  const improvement =
    h1us > h2us ? `${((1 - h2us / h1us) * 100).toFixed(0)}% faster` : `${((h2us / h1us - 1) * 100).toFixed(0)}% slower`
...
async function wsBench(
  url: string,
  msgFn: (i: number) => string,
  parseFn: (data: string) => boolean,
  n: number,
): Promise<number> {
...
async function runWSBenchmarks(): Promise<WSResult[]> {
  ...
  const vs_orpc =
    oUs > kUs * 1.05
      ? `${(oUs / kUs).toFixed(1)}x faster`
      : oUs < kUs * 0.95
        ? `${(kUs / oUs).toFixed(1)}x slower`
        : `~tied`
  const vs_h3 =
    hUs > kUs * 1.05
      ? `${(hUs / kUs).toFixed(1)}x faster`
      : hUs < kUs * 0.95
...
`
  ...
  console.log('\nHTTP/1.1:')
  for (const r of httpResults) console.log(`  ${r.name}: Silgi ${r.silgi_us}µs | H3 ${r.h3_us}µs | oRPC ${r.orpc_us}µs`)
  console.log('\nHTTP/2 vs HTTP/1.1:')
