...
async function main() {
  ...
  const httpTable = [
    ...
    `|---|---|---|---|`,
    ...httpResults.map(r =>
      `| ${r.name} | **${r.katman_us}µs** (${r.katman_rps}/s) | ${r.h3_us}µs (${r.h3_rps}/s) | ${r.orpc_us}µs (${r.orpc_rps}/s) |`
    ),
  ].join("\n");

  let md = await readFile("BENCHMARKS.md", "utf-8");
  md = md.replace("<!-- BENCH_DATE -->", date);
  md = md.replace("<!-- BENCH_ENV -->", env);
  md = md.replace("<!-- BENCH_PIPELINE -->", pipelineTable);
  md = md.replace("<!-- BENCH_HTTP -->", httpTable);
