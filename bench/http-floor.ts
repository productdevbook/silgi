...
function startORPC(port: number): Promise<Server> {
  const router = {
    ...
    echo: orpcOs.input(z.object({ message: z.string() })).handler(async ({ input }) => ({ echo: input.message })),
  }
...
async function main() {
  ...
  console.log(
    `  Bare + stringify:      ${fmt(bareStr.avg)}  ${bareStr.rps}/s  (+${((bareStr.avg - bare.avg) * 1000).toFixed(0)}µs)`,
  )
  ...
  console.log(
    `  oRPC overhead:   ${orpcOverhead.toFixed(0)}µs (${((orpcOverhead / bareUs) * 100).toFixed(0)}% of floor)`,
  )
  ...
  console.log(
    `  Katman:                      ${fmt(kat2.avg)}  ${kat2.rps}/s  (+${katOverhead2.toFixed(0)}µs overhead)`,
  )
  console.log(
    `  oRPC:                        ${fmt(orpc2.avg)}  ${orpc2.rps}/s  (+${orpcOverhead2.toFixed(0)}µs overhead)`,
  )
