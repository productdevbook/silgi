...
const link = createLink({ url: '', devalue: true })
...
async function fetchTodos() {
  ...
  responseInfo.value = `devalue (rich types) — ${ms}ms`
  loading.value = false
}
...
<template>
  <main class="mx-auto max-w-lg px-4 py-12 font-sans">
    ...
    <form class="mb-6 flex gap-2" @submit.prevent="addTodo">
      ...
      />
      <button type="submit" class="cursor-pointer rounded-lg bg-emerald-600 px-4 py-2 text-sm text-white hover:bg-emerald-700">
        Add
      </button>
    </form>
