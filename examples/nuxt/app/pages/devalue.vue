...
const client = useClient({ devalue: true })
...
async function fetchTodos() {
  ...
  responseInfo.value = `devalue (rich types) — ${(performance.now() - start).toFixed(1)}ms`
  loading.value = false
}
...
<template>
  <main class="mx-auto max-w-lg px-4 py-12 font-sans">
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h1 class="text-xl font-bold">devalue Protocol</h1>
    ...
    <form class="mb-6 flex gap-2" @submit.prevent="addTodo">
      ...
      />
      <button type="submit" class="rounded-lg bg-emerald-600 px-4 py-2 text-sm text-white">Add</button>
    </form>
