...
export default function App() {
  ...
  const health = useQuery(
    queryUtils.health.queryOptions({ input: undefined as never }),
  );

  const echo = useQuery(
    queryUtils.echo.queryOptions({ input: { msg: "Hello from React!" } }),
  );
  ...
  return (
    <main style={{ padding: "2rem", fontFamily: "system-ui", maxWidth: 600 }}>
      ...
      <p style={{ color: "#666" }}>
        Connects to any Katman server (standalone, Hono, Express, etc.)
      </p>
      ...
      <section style={{ marginTop: "1.5rem" }}>
        ...
        {greetMutation.data && (
          <p style={{ marginTop: "0.5rem" }}>
            {(greetMutation.data as { greeting: string }).greeting}
          </p>
        )}
        {greetMutation.error && (
          <p style={{ color: "red" }}>Error: {String(greetMutation.error)}</p>
        )}
      </section>
    </main>
  );
