...
description: Retry, circuit breaker, timeout, batching, deduplication, CSRF, and OpenTelemetry tracing for the Silgi client.
---
...
const link = withRetry(withCircuitBreaker(withTimeout(baseLink, { timeout: 5000 }), { failureThreshold: 5 }), {
  maxRetries: 3,
})
...
const link = withRetry(withCircuitBreaker(withTimeout(baseLink, { timeout: 5000 })))
```
