...
<Callout type='info'>
  `$output()` serves two purposes: it validates the return value at runtime, and it populates the response schema in
  the OpenAPI spec. Both benefits apply whenever you declare it.
</Callout>
