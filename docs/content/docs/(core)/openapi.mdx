...
const listUsers = s.$output(z.array(UserSchema)).$resolve(({ ctx }) => ctx.db.users.findMany())
```

<Callout type='info'>
  `$output()` serves two purposes: it validates the return value at runtime, and it populates the response schema in the
...
- [Code Generation](/docs/codegen) — generate Silgi routes _from_ an OpenAPI spec (the reverse direction)
