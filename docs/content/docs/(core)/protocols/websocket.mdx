...
<Callout type='info'>
  WebSocket-enabled procedures also work over HTTP. The `ws: true` flag adds WebSocket access, it doesn't remove HTTP
  access.
</Callout>
...
Both `attachWebSocket` and `createWSHooks` accept a context factory via the `context` option. The factory receives the `Peer` object:
...
`WSLink` handles connection lifecycle automatically — it connects on the first call, reconnects if the connection drops, and multiplexes concurrent calls over a single WebSocket.
