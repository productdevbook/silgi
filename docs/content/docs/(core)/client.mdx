...
<Callout type='warn'>
  If you use `$route({ path })` on any procedure but forget to pass `routes` to the link, the client will get 404
  errors. The `routes` option is only optional when **no** procedures use custom paths.
</Callout>
