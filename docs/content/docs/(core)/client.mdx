...
| `router`     | `RouterDef`                          | `undefined` | Router definition — **required** when procedures use `$route({ path })` |

### Route resolution

When any procedure uses `$route({ path, method })`, you **must** pass the router definition to the link. Without it, the client sends requests to the tree path (e.g. `POST /commerce/orders/list`) which doesn't exist on the server — only the custom path (`GET /api/orders`) is registered.
...
const link = createLink({
  url: 'http://localhost:3000',
  router: appRouter, // required when using $route({ path })
})
```
...
<Callout type='warn'>
  If you use `$route({ path })` on any procedure but forget to pass `router` to the link, the client will get 404 errors. The `router` option is only optional when **no** procedures use custom paths.
</Callout>
