...
| `router`     | `RouterDef`                          | `undefined` | Router definition — resolves `$route({ path })` on the client |

### Route resolution

When procedures use `$route({ path, method })`, pass the router definition to the link so the client calls the correct URL:
...
Now `client.commerce.orders.list()` sends `GET /api/orders` instead of `POST /commerce/orders/list`. Path parameters like `:id` are automatically extracted from the input and substituted into the URL:
...
<Callout type='info'>
  Without the `router` option, the client falls back to the default tree path (`POST /commerce/orders/list`). The `router` option is only needed when procedures use `$route({ path })`.
</Callout>
