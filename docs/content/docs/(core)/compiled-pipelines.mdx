...
<Mermaid chart="graph TD\n  S[selectGuardRunner] --> C{guard count}\n  C -->|0| R0[runGuards0 — no-op]\n  C -->|1| R1[runGuards1 — direct call]\n  C -->|2| R2[runGuards2 — 2 direct calls]\n  C -->|3| R3[runGuards3 — 3 direct calls]\n  C -->|4| R4[runGuards4 — 4 direct calls]\n  C -->|5+| RN[runGuardsN — loop fallback]" />
...
<Mermaid chart="graph LR\n  REQ[Request arrives] --> ACQ{Pool empty?}\n  ACQ -->|No| POP[Pop from pool]\n  ACQ -->|Yes| NEW[Object.create null]\n  POP --> USE[Guards + resolver use ctx]\n  NEW --> USE\n  USE --> DONE[Request completes]\n  DONE --> WIPE[Wipe all properties]\n  WIPE --> RET{Pool full?}\n  RET -->|No, under 128| PUSH[Return to pool]\n  RET -->|Yes| GC[Let GC collect]" />
...
<Mermaid chart="graph TD\n  R[s.router] --> W[Walk procedure tree]\n  W --> U1[users.list] --> A1[addRoute POST /users/list]\n  W --> U2[users.get] --> A2[addRoute POST /users/get]\n  W --> U3[users.:id] --> A3[addRoute GET /users/:id]\n  A1 --> RT[rou3 radix tree]\n  A2 --> RT\n  A3 --> RT\n  RT --> M[findRoute — O path length]" />

Route matching is O(path length), not O(route count). Custom routes defined with `.$route({ path: '/users/:id', method: 'GET' })` are also compiled into the same tree, including wildcard and parameterized segments.
