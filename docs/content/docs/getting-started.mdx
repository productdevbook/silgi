...
<Tabs items={['npm', 'pnpm', 'bun']}>
  <Tab value='npm'>```bash npm install katman zod ```</Tab>
  <Tab value='pnpm'>```bash pnpm add katman zod ```</Tab>
  <Tab value='bun'>```bash bun add katman zod ```</Tab>
</Tabs>
...
// @noErrors
import { k, query, mutation, guard, router } from './rpc'
...
- [Procedures](/docs/procedures) — the two forms: short and full config
