...
<Tabs items={['pnpm', 'npm', 'bun']}>
  <Tab value='pnpm'>
  ```bash
  pnpm add silgi drizzle-orm
  ```
  </Tab>
  <Tab value='npm'>
  ```bash
  npm install silgi drizzle-orm
  ```
  </Tab>
  <Tab value='bun'>
  ```bash
  bun add silgi drizzle-orm
  ```
  </Tab>
</Tabs>
...
const authDb = instrumentDrizzle(
  drizzle(process.env.AUTH_DATABASE_URL!),
  { dbName: 'auth', peerName: 'db.example.com', peerPort: 5432 },
)

const ecommerceDb = instrumentDrizzle(
  drizzle(process.env.ECOMMERCE_DATABASE_URL!),
  { dbName: 'ecommerce', peerName: 'db.example.com', peerPort: 5432 },
)
```
