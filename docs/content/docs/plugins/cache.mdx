...
<Callout type='info'>
  The cache key is computed from the serialized input using [ohash](https://github.com/unjs/ohash). Two calls with the same input always produce the same key, regardless of property order.
</Callout>
