...
```ts twoslash
// @noErrors
import { cacheQuery } from 'katman/cache'

const listUsers = k.$use(cacheQuery({ maxAge: 60 })).$resolve(({ ctx }) => ctx.db.users.findMany())
```
...
```ts twoslash
// @noErrors
import { cacheQuery, invalidateQueryCache } from 'katman/cache'
...
By default, cache lives in memory. For production, plug in any storage backend via [unstorage](https://unstorage.unjs.io).

<Tabs items={['pnpm', 'npm', 'bun']}>
  <Tab value='pnpm'>```bash pnpm add unstorage ```</Tab>
  <Tab value='npm'>```bash npm install unstorage ```</Tab>
  <Tab value='bun'>```bash bun add unstorage ```</Tab>
</Tabs>
...
```ts twoslash
// @noErrors
import { setCacheStorage } from 'katman/cache'
