...
<Callout type='info'>
  Katman has zero OpenTelemetry dependency. The plugin defines a lightweight `Tracer` and `Span` interface. You bring
  `@opentelemetry/api` yourself, which means Katman never pins you to a specific OTel version.
</Callout>
