...
```ts twoslash
// @noErrors
const listUsers = s.$use(rateLimit).$resolve(({ ctx }) => ctx.db.users.findMany())
```
...
```ts twoslash
// @noErrors
const listUsers = s.$use(rateLimit).$resolve(({ ctx }) => {
...
const listUsers = s.$use(publicLimit).$resolve(...)
const createUser = s.$use(auth, authLimit).$resolve(...)
```
