...
```ts twoslash
// @noErrors
const listUsers = k.$use(rateLimit).$resolve(({ ctx }) => ctx.db.users.findMany())
```
...
```ts twoslash
// @noErrors
const listUsers = k.$use(rateLimit).$resolve(({ ctx }) => {
...
const listUsers = k.$use(publicLimit).$resolve(...)
const createUser = k.$use(auth, authLimit).$resolve(...)
```
