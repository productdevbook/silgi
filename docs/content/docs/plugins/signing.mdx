...
```ts twoslash
// @noErrors
import { sign, unsign } from 'silgi/plugins'
...
// @noErrors
import { sign, unsign } from 'silgi/plugins'
import { setCookie, getCookie } from 'silgi/cookies'
...
if (!userId) throw new SilgiError('UNAUTHORIZED')
```
...
```ts twoslash
// @noErrors
import { encrypt, decrypt } from 'silgi/plugins'
