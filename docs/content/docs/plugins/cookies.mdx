...
Lightweight cookie utilities for Silgi handlers. No dependencies. Works with both `serve()` and `handler()`.

## Reading cookies

```ts twoslash
// @noErrors
import { getCookie, parseCookies } from 'silgi/cookies'
...
```ts twoslash
// @noErrors
import { setCookie } from 'silgi/cookies'
...
```ts twoslash
// @noErrors
import { deleteCookie } from 'silgi/cookies'
