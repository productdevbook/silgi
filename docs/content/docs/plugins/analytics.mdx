...
The dashboard is a self-contained HTML page with vanilla JS — no external dependencies. It auto-refreshes every 2 seconds and is served at `/analytics`.

**Overview tab:**

- Total requests, req/s, error rate, and average latency stat cards
- Request rate sparkline chart with error markers
- Per-procedure table with sortable columns (count, errors, error rate, latency percentiles)

**Errors tab:**

- Chronological error log with expandable details
- Full input, headers, stack trace, and traced spans for each error
