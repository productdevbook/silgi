...
const createUser = s.$use(auth, limit).$resolve(({ input, ctx }) => ctx.db.users.create(input))

const createPost = s.$use(auth, limit).$resolve(({ input, ctx }) => ctx.db.posts.create(input))
```
