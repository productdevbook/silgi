...
const createUser = k.$use(auth, limit).$resolve(({ input, ctx }) => ctx.db.users.create(input))

const createPost = k.$use(auth, limit).$resolve(({ input, ctx }) => ctx.db.posts.create(input))
```

<Callout>
  The guard only checks `Content-Length`. It does not buffer or measure the actual body stream. For streaming uploads,
