...
<Callout>
  The `'local'` option requires `@scalar/api-reference` as a dependency. If the package is not found, Katman falls back
...
| `cache`              | Cache-Control header for query responses       |
...
<Callout type='info'>
  Caching only applies to query procedures. Mutations and subscriptions never get Cache-Control headers, even if you set
  `cache` on them.
</Callout>
...
Return a `ReadableStream` for binary streaming. Katman sends it with `application/octet-stream`:

```ts twoslash
// @noErrors
const exportCsv = k.query({
  resolve: async ({ ctx }) => {
