...
const listUsers = k.$route({ cache: 60 }).$resolve(({ ctx }) => ctx.db.users.findMany())

// Full control: custom Cache-Control value
const getConfig = k.$route({ cache: 'public, max-age=300, stale-while-revalidate=60' }).$resolve(() => loadConfig())
```
...
```ts twoslash
// @noErrors
const downloadPdf = k.$input(z.object({ id: z.string() })).$resolve(async ({ input, ctx }) => {
...
```ts twoslash
// @noErrors
const exportCsv = k.$resolve(async ({ ctx }) => {
