...
export const auth = betterAuth({
  ...
  plugins: [silgiTracing({ createAuthMiddleware })],
})
```
...
<Callout type='warn'>
  `request.__silgiCtx = ctx` must be set before calling `auth.handler(request)`. Without it, the plugin has no access to
...
// @noErrors
import { silgi } from 'silgi'
import { withSilgiCtx } from 'silgi/better-auth'
import { auth } from './auth'

const s = silgi()

const me = s.$resolve(async ({ ctx, request }) => {
  return withSilgiCtx(ctx, () => auth.api.getSession({ headers: request.headers }))
})
```
