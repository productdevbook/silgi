...
<Callout type='info'>
  `fail()` is only typed when you provide the `errors` map. Without it, `fail()` still works but accepts any string
  code.
</Callout>
...
<Callout type='info'>
  The simple `k.guard(fn)` form still works. The config form `k.guard({ errors, fn })` is opt-in — use it when you want the guard's errors to be part of the typed contract.
</Callout>
...
- [Procedures](/docs/procedures) — the full config form where error maps are defined
