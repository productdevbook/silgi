...
const listUsers = s.$input(ListInput).$resolve(() => {} as any)
const createUser = s.$input(UserInput).$resolve(() => {} as any)

export const contractRouter = s.router({
...
const listUsers = s.$input(ListInput).$resolve(({ input, ctx }) => ctx.db.users.findMany({ take: input.limit ?? 10 }))
...
```ts twoslash
// @noErrors
import type { InferContractClient } from 'silgi'
import type { AppRouter } from 'api-contract'
