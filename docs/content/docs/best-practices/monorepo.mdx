...
const listUsers = k.$input(ListInput).$resolve(() => {} as any)
const createUser = k.$input(UserInput).$resolve(() => {} as any)
...
const listUsers = k.$input(ListInput).$resolve(({ input, ctx }) => ctx.db.users.findMany({ take: input.limit ?? 10 }))
