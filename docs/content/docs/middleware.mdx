...
```ts twoslash
// @noErrors
const me = s.$use(auth).$resolve(({ ctx }) => {
...
```ts twoslash
// @noErrors
const withSentry = s.wrap(async (ctx, next) => {
  try {
    return await next()
  } catch (error) {
...
const listUsers = s.$use(auth, timing).$resolve(({ ctx }) => ctx.db.users.findMany())

const deleteUser = s.$use(auth, adminOnly, timing).$resolve(({ ctx, input }) => ctx.db.users.delete(input.id))
```
...
const listUsers = s.$use(logging).$resolve(({ ctx }) => ctx.db.users.findMany())
```
...
const getUser = s.$use(renameUserId).$resolve(({ input }) => db.users.find(input.id))
```
