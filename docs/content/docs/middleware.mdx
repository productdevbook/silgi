...
```ts twoslash
// @noErrors
const me = k.$use(auth).$resolve(({ ctx }) => {
...
The simple `guard(fn)` form still works exactly as before. Use `guard({ errors, fn })` when you want the guard's errors to be part of the typed API contract. See [Typed Errors](/docs/errors#typed-guard-errors) for more details.
...
const listUsers = k.$use(auth, timing).$resolve(({ ctx }) => ctx.db.users.findMany())

const deleteUser = k.$use(auth, adminOnly, timing).$resolve(({ ctx, input }) => ctx.db.users.delete(input.id))
```
...
const listUsers = k.$use(logging).$resolve(({ ctx }) => ctx.db.users.findMany())
```
...
This is different from the global [lifecycle hooks](/docs/server#lifecycle-hooks) on the katman instance — `lifecycleWrap` applies per-procedure via `.$use()`.
...
const getUser = k.$use(renameUserId).$resolve(({ input }) => db.users.find(input.id))
```
...
- [Procedures](/docs/procedures) — learn about the short form and builder for defining endpoints
