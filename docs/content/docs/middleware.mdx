---
title: Middleware
description: Guards and wraps — the two kinds of middleware in Silgi, when to use each, and how they execute.
---
...
<Callout type='info'>
  Guards can be synchronous or asynchronous. When they're sync, Silgi skips the async overhead entirely — no Promises
  are created.
</Callout>
...
export const adminOnly = guard((ctx) => {
  if (ctx.user.role !== 'admin') throw new SilgiError('FORBIDDEN')
})
...
This is different from the global [lifecycle hooks](/docs/server#lifecycle-hooks) on the silgi instance — `lifecycleWrap` applies per-procedure via `.$use()`.
