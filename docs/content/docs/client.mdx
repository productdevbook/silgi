...
The Katman client gives you a fully typed proxy that mirrors your server's router structure. You call procedures like local functions, and TypeScript knows every input and output type.
...
Server errors are reconstructed as `KatmanError` instances on the client. You can catch them with try/catch:
...
```ts twoslash
// @noErrors
import { createClient, DynamicLink } from 'katman/client'
...
Combine multiple Katman clients into a single object. Each client can point to a different server — the merged result keeps full type safety:
...
Wrap any link with lifecycle hooks. Unlike ofetch interceptors (which are HTTP-level), these run at the Katman procedure level and receive the procedure path and input:
