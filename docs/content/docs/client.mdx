...
A link tells the client *how* to send requests to the server. The built-in link uses [ofetch](https://github.com/unjs/ofetch) for retries, timeouts, and JSON parsing:
...
const { error, data, isError, isSuccess } = await safe(
