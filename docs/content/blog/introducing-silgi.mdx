...
Every Silgi server can enable a live monitoring dashboard by passing an `analytics` options object. Request tracing, error logs with full context, session tracking, span waterfall — no Prometheus, no Grafana, no external infrastructure.
