  ...
  return (
    <HomeLayout {...baseOptions()}>
      <div className="k-sans max-w-6xl mx-auto k-landing">
        ...
        <Heading
          label="Architecture"
          title="Build your API in composable layers"
        />
        <ArchitectureGrid />
        ...
        <Heading
          label="Wire formats"
          title="Three protocols, automatic negotiation"
        />
        <ProtocolGrid />
          ...
          <div className="flex flex-col justify-between gap-16 p-6 sm:p-10">
            <div className="flex flex-col gap-5 items-center md:items-start text-center md:text-left">
              ...
              <p className="text-[var(--k-text)] md:text-lg max-w-[27rem] text-pretty font-normal leading-relaxed">
                Silgi is a type-safe RPC framework with compiled pipelines
...
const CODE = [
  ...
  '<span class="kw">const</span> auth = s.<span class="fn">guard</span>(<span class="kw">async</span> (ctx) =&gt; {',
  ...
  '<span class="kw">const</span> users = s',
  ...
  '<span class="kw">const</span> appRouter = s.<span class="fn">router</span>({ users })',
  "",
  's.<span class="fn">serve</span>(appRouter, {',
    ...
    <section>
      <W ticks className="grid grid-cols-2">
        {[1, 2].map((i) => (
          ...
          >
            <span className="k-mono text-[11px] uppercase tracking-[.2em]">
              Special Sponsor
            </span>
  ...
  return (
    <section>
      <W border className="px-6 sm:px-10 py-16 lg:py-20">
        <span className="k-mono text-xs uppercase tracking-[.3em] text-[var(--k-dim)] block mb-4">
          {label}
        </span>
    ...
    <section>
      <W ticks className="grid sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Cell bt bl>
          <CT
            t="Lazy loading"
            d="lazy(() => import('./routes/users')) for code splitting. Cached after first load."
          />
        </Cell>
        <Cell bt bl>
          <CT
            t="Contract-first"
            d="Define the API shape, share with frontend, implement on backend. Types enforced."
          />
        </Cell>
      </W>
    </section>
    ...
    <section>
      <W ticks className="grid sm:grid-cols-2 lg:grid-cols-4">
        <Cell>
          <CT
            t="React Actions"
            d="createAction() returns [error, data] tuples. FormData with bracket notation."
          />
        </Cell>
        <Cell bl>
          <CT
            t="TanStack Query"
            d="queryOptions, mutationOptions, queryKey. React, Vue, Solid, Svelte."
          />
        </Cell>
        <Cell bl>
          <CT
            t="AI SDK"
            d="routerToTools() — LLMs call your procedures through function calling."
          />
        </Cell>
        <Cell bl>
          <CT
            t="Fastify"
            d="Register as a plugin alongside existing REST routes."
          />
        </Cell>
      </W>
    </section>
    ...
    <section>
      <W ticks className="grid md:grid-cols-2">
        <Cell>
          <CT
            t="serve()"
            d="One-line Node.js server. Auto port finding, HTTP/2 with TLS, WebSocket on same port."
          />
          <div className="mt-5 rounded-lg border border-[var(--k-line)] bg-[var(--k-raised)] p-4 k-mono text-[12px] text-[var(--k-dim)]">
            <p>s.serve(router, {"{"}</p>
            <p>
              &nbsp; port:{" "}
              <span style={{ color: "var(--k-code-nr)" }}>3000</span>,
            </p>
            <p>
              &nbsp; scalar:{" "}
              <span style={{ color: "var(--k-code-kw)" }}>true</span>,
            </p>
            <p>
        ...
        <Cell bl>
          <CT
            t="handler()"
            d="Fetch API handler — works everywhere. Content negotiation is automatic."
          />
          <div className="mt-5 grid grid-cols-2 gap-3">
            {["Node.js", "Bun", "Deno", "Cloudflare"].map((r) => (
              <div
                key={r}
                className="rounded-lg border border-[var(--k-line)] px-4 py-2.5 text-center"
              >
                <span className="text-sm text-[var(--k-text)] font-normal">
                  {r}
                </span>
              </div>
            ))}
    ...
    <section>
      <W ticks className="grid sm:grid-cols-2 lg:grid-cols-4">
        <Cell>
          <CT
            t="CORS"
            d="corsHeaders() — string, array, or dynamic origin matching."
          />
        </Cell>
        <Cell bl>
          <CT
            t="OpenTelemetry"
            d="otelWrap(tracer) — each procedure call becomes a span."
          />
        </Cell>
        <Cell bl>
          <CT
            t="Pino"
            d="loggingHooks() — structured request/response/error logging."
          />
        </Cell>
        <Cell bl>
          <CT
            t="Rate Limiting"
            d="Sliding window guard. In-memory or custom backend."
          />
        </Cell>
      </W>
    </section>
    ...
    <section>
      <W border className="p-6 sm:p-10">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-10">
          <div>
            <span className="k-mono text-xs uppercase tracking-[.3em] text-[var(--k-dim)] block mb-3">
              Sponsors
            </span>
        ...
        <div className="mb-6">
          <span className="k-mono text-[11px] uppercase tracking-[.25em] text-[var(--k-dim)] block mb-3">
            Platinum
          </span>
          <a
            href={link}
...
function FooterCTA() {
  return (
    <section className="relative k-grain overflow-hidden">
      <W
        border
        className="relative px-6 sm:px-10 py-24 lg:py-32 text-center flex flex-col items-center"
      >
        ...
        <p className="text-[var(--k-text)] max-w-sm mb-10 font-normal leading-relaxed">
          Prepare for a development environment that can finally keep pace with
...
function CT({ t, d }: { t: string; d: string }) {
  return (
    <div className="flex flex-col gap-2">
      ...
      <p className="text-sm text-[var(--k-dim)] font-normal leading-relaxed max-w-[28rem] text-pretty">
        {d}
      </p>
    </div>
  );
...
function Badge({ c, t, b }: { c: string; t: string; b: string }) {
  return (
    <div className="flex items-center gap-2.5">
      ...
      <h5 className="font-medium text-[var(--k-fg)]">{t}</h5>
      <span
        className="k-mono text-[11px] uppercase tracking-[.15em] ml-auto"
        style={{ color: c }}
      >
        {b}
      </span>
