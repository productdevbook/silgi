...
export function RequestChart({ timeSeries }: RequestChartProps) {
  ...
  if (chartData.length === 0) {
    return (
      <div className='flex h-36 items-center justify-center text-xs text-muted-foreground'>Waiting for data...</div>
    )
  }
  ...
  return (
    <ChartContainer config={chartConfig} className='h-52 w-full'>
      <ComposedChart data={chartData} margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
        ...
        />
        <Bar dataKey='errors' fill='var(--color-errors)' fillOpacity={0.6} radius={[1, 1, 0, 0]} barSize={4} />
      </ComposedChart>
    </ChartContainer>
  )
}
