...
export function Requests({ requests, navigate, initialProcedure }: RequestsProps) {
  ...
  const procedures = useMemo(() => getProcedureOptions(requests.flatMap((e) => e.procedures.map((p) => p.procedure))), [requests])
  ...
  const filtered = useMemo(() => {
    ...
    result.sort((a, b) => {
      switch (sortKey) {
        ...
        case 'procedure': return dir * (a.procedures[0]?.procedure ?? '').localeCompare(b.procedures[0]?.procedure ?? '')
        case 'status': return dir * (a.status - b.status)
        case 'duration': return dir * (a.durationMs - b.durationMs)
        case 'spans': return dir * (a.procedures.reduce((s, p) => s + p.spans.length, 0) - b.procedures.reduce((s, p) => s + p.spans.length, 0))
      }
    })
    return result
  ...
  const clearFilters = useCallback(() => { setQuery(''); setProcedure('all'); setStatus('all'); setLatency('all') }, [])
    ...
    <div className='flex min-h-full flex-col'>
      ...
      <div className='flex flex-col gap-2 border-b px-4 py-3 lg:flex-row lg:items-center lg:justify-between'>
        <div className='flex flex-1 flex-wrap items-center gap-2'>
          <SearchField value={query} onChange={(e) => setQuery(e.target.value)} placeholder='Search procedures...' className='sm:max-w-56' />
          ...
          <ToggleGroup value={status} onValueChange={(v) => setStatus((v as RequestStatusFilter) || 'all')} variant='outline' size='sm' spacing={1}>
          ...
          <ToggleGroup value={latency} onValueChange={(v) => setLatency((v as RequestLatencyFilter) || 'all')} variant='outline' size='sm' spacing={1}>
            <ToggleGroupItem value='all'>All</ToggleGroupItem>
      ...
      {filtered.length > 0 && (
        <div className='border-b px-5'>
          <ChartContainer config={CHART_CONFIG} className='h-10 w-full'>
            <BarChart data={filtered.map((r, i) => ({ i, d: r.durationMs, err: r.status >= 400 }))} margin={{ top: 4, right: 0, bottom: 0, left: 0 }} barGap={1}>
              <ChartTooltip content={({ active, payload }) => {
                ...
                return (
                  <div className='rounded-md border bg-background px-2.5 py-1.5 text-xs shadow-sm'>
                    ...
                    <div className='text-muted-foreground'>{fmtMs(r.durationMs)} &middot; {r.status}</div>
                  </div>
                )
              }} />
              <Bar dataKey='d' radius={[2, 2, 0, 0]} cursor='pointer' onClick={(_, idx) => setSelectedIdx(selectedIdx === idx ? null : idx)}>
                {filtered.map((r, i) => (
                  <Cell key={i} fill={r.status >= 400 ? 'var(--color-destructive)' : 'var(--chart-1)'} fillOpacity={selectedIdx === i ? 1 : r.status >= 400 ? 0.6 : 0.3} />
                ))}
              </Bar>
            </BarChart>
          </ChartContainer>
      ...
      {filtered.length === 0 ? (
        <div className='flex min-h-48 flex-col items-center justify-center gap-1 text-center'>
          ...
          <p className='text-xs text-muted-foreground'>{hasActiveFilters ? 'Adjust filters.' : 'Waiting for traffic.'}</p>
        ...
        <div className='flex flex-1 overflow-hidden'>
          ...
          <div className={cn('flex-1 overflow-y-auto', selectedReq && 'hidden xl:block')}>
            ...
            <div className='flex items-center gap-2 border-b px-5 py-1.5 text-[10px] uppercase tracking-wider text-muted-foreground'>
              <SortCol label='Time' sortKey='time' currentKey={sortKey} asc={sortAsc} onSort={handleSort} className='w-14' />
              <span className='w-2' />
              <span className='w-8'>Method</span>
              <SortCol label='Procedure' sortKey='procedure' currentKey={sortKey} asc={sortAsc} onSort={handleSort} className='flex-1' />
              <SortCol label='Status' sortKey='status' currentKey={sortKey} asc={sortAsc} onSort={handleSort} className='w-12' />
              <span className='hidden w-28 lg:block'>Waterfall</span>
              <SortCol label='Duration' sortKey='duration' currentKey={sortKey} asc={sortAsc} onSort={handleSort} className='w-14 text-right' />
              <SortCol label='Spans' sortKey='spans' currentKey={sortKey} asc={sortAsc} onSort={handleSort} className='w-8 text-right' />
            ...
            {filtered.map((entry, idx) => {
              ...
              return (
                <div
                  key={entry.id}
                  className={cn('flex cursor-pointer items-center gap-2 border-b border-dashed px-5 py-2 hover:bg-muted/20', selectedIdx === idx && 'bg-primary/5')}
                  onClick={() => setSelectedIdx(selectedIdx === idx ? null : idx)}
                >
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <span className='w-14 shrink-0 text-[11px] tabular-nums text-muted-foreground'>{fmtRelativeTime(entry.timestamp)}</span>
                    </TooltipTrigger>
                    <TooltipContent side='right' className='text-xs'>{fmtTime(entry.timestamp)}</TooltipContent>
                  </Tooltip>
                  ...
                  <span className='min-w-0 flex-1 truncate font-mono text-[11px] font-semibold'>
                    ...
                    {entry.isBatch && <Badge variant='outline' className='ml-1.5 text-[9px]'>batch</Badge>}
                  </span>
                  <Badge variant={isError ? 'destructive' : 'secondary'} className='w-12 justify-center text-[9px]'>{entry.status}</Badge>
                  <div className='hidden w-28 lg:block'>
                    <div className='h-1.5 w-full rounded-full bg-muted'>
                      <div className={cn('h-full rounded-full', isError ? 'bg-destructive/50' : 'bg-primary/40')} style={{ width: `${Math.max(durationPct, 2)}%` }} />
                    </div>
                  </div>
                  <span className='w-14 shrink-0 text-right font-mono text-[11px] tabular-nums text-muted-foreground'>{fmtMs(entry.durationMs)}</span>
                  <span className='w-8 shrink-0 text-right font-mono text-[9px] text-muted-foreground/50'>{spanCount || ''}</span>
                  <Badge
                    variant='outline'
...
function RequestDetailPanel({ req, onClose, navigate }: { req: RequestEntry; onClose: () => void; navigate: (page: string, id?: string) => void }) {
  return (
    <>
      <div className='flex items-center gap-2 border-b px-4 py-2'>
        <span className={cn('size-1.5 shrink-0 rounded-full', req.status >= 400 ? 'bg-destructive' : 'bg-emerald-500')} />
        <span className='flex-1 truncate font-mono text-[11px] font-semibold'>{req.procedures.map((p) => p.procedure).join(', ')}</span>
      ...
      <div className='flex-1 overflow-y-auto'>
          ...
          <div key={idx}>
            ...
            {proc.input !== undefined && proc.input !== null && (
              <PanelSection label='Input'>
                <pre className='overflow-x-auto whitespace-pre-wrap rounded-md bg-muted/30 p-2.5 font-mono text-[10px] leading-relaxed'>{JSON.stringify(proc.input, null, 2)}</pre>
              </PanelSection>
            )}
            {proc.output !== undefined && proc.output !== null && (
              <PanelSection label='Output'>
                <pre className='max-h-36 overflow-auto whitespace-pre-wrap rounded-md bg-muted/30 p-2.5 font-mono text-[10px] leading-relaxed'>{JSON.stringify(proc.output, null, 2)}</pre>
              </PanelSection>
            )}
            {proc.error && (
        ...
        <div className='px-4 py-3'>
          <Button variant='outline' size='xs' className='w-full' onClick={() => navigate('requests', String(req.id))}>Open full detail</Button>
        </div>
      </div>
    </>
  )
